...
#             print "px",px
            res = -log(px)
            #res = -norm.logpdf(var, m, s)
            #res = -log(t.pdf(var, len(train)-1, loc=m, scale=s)) 
            return res
        except:
            return 0
...
if __name__=='__main__':
    fname = 'test'
    term = 5 # time window of training
    smooth = 1
    kfad = KFAnomalyDetection(term,smooth,2,0,0,20)
    datalist = []
#     with open(fname, 'r', 1000) as f:
#         for line in f:
#             data = float(line.strip())
    ...
    print "change_score",kfad.change_score_smooth
    of.close()
    
    rng = range( len(dlist.tolist()) )
    plt.plot(rng,dlist,label=u"predicted data")
    plt.show()
    plt.plot(rng,kfad.change_score_smooth,label=u"predicted data")
    plt.show()