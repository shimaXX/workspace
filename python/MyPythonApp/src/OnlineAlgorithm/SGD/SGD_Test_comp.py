...
class SGD_Test(object):
    ...
    def test(self):
        ...
        with open(self.testdata_fname,'r') as testf:
            ...
            for line in testf:
                ...
                y = line.strip().split(' ')[0] # yは文字列で取得される
                ...
                for kv in data: # 推定量の算出
                    ...
                    comp += self.calc_iter_comp(int(k),float(v),bitmask)
                res = self.calc_comped_pred( self.calc_result(m), comp )
                self.get_table(y, res)
                predict.append(res)
        return predict, self.ans
        ...
        with open(self.modelfile,'r') as modelf:
            ...
            for line in modelf:
                if cnt==0:
                    ...
                    self.sig2 = line.strip().split(' ')
                    self.sig2 = np.array(self.sig2, dtype=np.float64)
                    print self.sig2
                cnt += 1
    ...
    def calc_iter_comp(self,k,x,bitmask):
        if self.type=='log':
            return self.sig2[k&bitmask]*x*x
        elif self.type=='hinge':
            pass

    def calc_comped_pred(self,p,comp):
#        print 'level:',(0.5-p)*p*(1-p)*comp
        if p > 0.5:
            return p
        else:
            return p + (0.5-p)*p*(1-p)*comp # pが0.5より大きい場合、マイナスになる
