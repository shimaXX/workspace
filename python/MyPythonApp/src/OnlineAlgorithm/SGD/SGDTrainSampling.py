...
#        self.l = np.zeros(self.bitmask) # parameter of updating learning rate
        ...
        self.alpha=100 # active samplingのためのパラメタ。採用サンプル数が多い
...
#             return -ez / (ez + 1.0) # -yx+1/(1+e^(-m))*yx    
    ...
    def selective_sampler(self, y, pred):
        r = sum(self.Adiag**(-1)*self.features**2)
        theta2 = self.alpha*r*math.log(1+self.t_select)
        if theta2 >= pred**2:
            self.Adiag += self.features**2
            if abs(pred)>1:
                sgn_pred = 1 if pred>=0 else -1
                w = self.weight - sgn_pred*(abs(pred)-1)/r*self.Adiag**(-1)*self.features
            else:
                w = self.weight
            self.weight = self.Adiag**(-1)*(self.Adiag*w+y*self.features)
            self.t += 1
