...
class crawler:
    ...
    def getentryid(self,table,field,value,createnew=True):
        res=self.con.execute(
                "select rowid from %s where %s='%s'" % (table,field,value)).fetchone()
        if res==None:
    ...
    # 個々のページをインデックスする
    def addtoindex(self,url,wordlist):
        if self.isindexed(url): return
        print 'Indexing' + url
                                
        # url毎に単語リストをDBに格納
        self.con.execute( "insert into wordvector values(?,?,?,?)" , \
                            (url, '', pickle.dumps(wordlist), '') )
        self.dbcommit()
        ...
        for i in xrange(depth):
            ...
            for page in pages:
                try:
                    ...
                    continue
                en = chardet.detect(response)['encoding'] # 文字コードの取得
...
#    pagelist=['http://search.rakuten.co.jp/search/mall/-/100968/?l-id=gt_bty_l_xs'] # 'http://yahoo.co.jp'
    ...
    fname = 'D:/workspace/python/MyPythonApp/works/input/learnerURL.txt'
    crawler=crawler('learner.db')
