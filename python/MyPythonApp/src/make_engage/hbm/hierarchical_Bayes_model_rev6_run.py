...
#
# データインポートファイルパスの定義
inpt_Z_filename = 'C:/Users/n_shimada/Desktop/real_stage/Galster_CRM_visit_sliced_rev2.csv'
inpt_D_filename = 'C:/Users/n_shimada/Desktop/real_stage/Galster_CRM_D_sliced_rev2.csv'
inpt_payment_filename = 'C:/Users/n_shimada/Desktop/real_stage/Galster_CRM_pymData_sliced_rev2.csv'
inpt_C_filename = 'C:/Users/n_shimada/Desktop/real_stage/Galster_CRM_C_data_sliced_rev2.csv'
inpt_visit_filename = 'C:/Users/n_shimada/Desktop/real_stage/Galster_CRM_sliced_rev2.csv'
#
### データinput-------------------
# Z1=訪問間隔データの取得
...
def calculate_spending_habits_param_delta((hh, u, Xs, Sita_dlt, Hdlt, Vsita_dlt, Ztld)):  # step4のΘ_δの計算
    ...
    # 効用値の誤差計算(θの尤度計算)
    Lsita = sum( (u[hh,:] - np.sum(Ztld[:,:,hh]*Xs[hh,:,:], axis = 0) )**2  )
    # 現状のθを確保する
...
#
        ...
        INV = np.array( pool.map(calc_INV, 
                         ((hh, Sita_dlt, Sita_lmbd) for hh in xrange(nhh))) )
        pool.close()
        pool.join()
...
#    cProfile.run("worker(int(sys.argv[1]), int(sys.argv[2]))")
    ...
    for hh in xrange(nhh):
        ...
        for t in xrange(1):
            nums = [str(udraw[hh,t,p]) for p in xrange(RP)]
