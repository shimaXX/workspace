...
Ztld=[]
## 顧客の消費傾向Data
INI_INV = np.array([[np.double(0)]*TIMES for hh in xrange(nhh)])
...
Ztld[(k-1)+SEASONALYTY+nz-1,:,:] = hbm.calc_INV(TIMES, p, C, INI_INV, dlt, lmbd).T
...
def calc_INV((hh, INV, dlt, lmbd)):
    for t in xrange(1,TIMES):
        #print "sita", dlt[hh]*C[hh] + (INV[hh,t-1])**lmbd[hh]
        Cons = INV[hh,t-1]*dlt[hh]*C[hh]/( dlt[hh]*C[hh] + INV[hh,t-1]**lmbd[hh] )
...
#
        ...
        INV = np.array( pool.map(calc_INV, 
                         ((hh, INI_INV, np.exp(Sita_dlt), Sita_lmbd) for hh in xrange(nhh))) )
        pool.close()
        pool.join()
