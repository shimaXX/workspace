# coding: utf-8
"""
全ての分布との距離を加味した最適化を行うこと
交換が起きた時にindexがズレる件の修正
...
class Cluto:
        ...
        for clustor_idx in xrange(clustor_number): # original cluster outer loop
            for idx_label, student in enumerate(clustors[clustor_idx]): # original cluster inner loop
                for change_clustor_idx in xrange(clustor_idx+1,clustor_number): # competitor cluster outer loop
                    ...
                    other_clust = range(clustor_number)
                    other_clust.pop(clustor_idx)
                    other_clust = clustors[other_clust].tolist()
                    dist_orig = self.total_mic(this_clustor_data, other_clust)
                    ...
                    for idx in target_idx: # competitor cluster inner loop(sampled)
                        ...
                        this_temporary.insert(idx_label,tmp_change)
                        
                        other_clust.pop(change_clustor_idx-1)
                        other_clust.append(change_tmp_data)
                        
                        dist_change = self.total_mic(this_temporary, other_clust)
                        ...
                        if min_dist > dist_change: # 全てのクラスとの距離の和を計算する必要があるかも...
                            ...
                            labels[clustor_idx].insert(idx_label,change_tmp_label)
    ...
    def total_mic(self,this_clustor,othre_clustors):
        dist = 0
        for oc in othre_clustors:
            dist += mic(this_clustor, oc)
        return dist
...
if __name__=='__main__':
    ...
    black_list = {1:[2,3,4,5,120,130,12,11,10], 2:[1,10,7,50,20,30]}
    ...
    for _ in xrange(3):
        ...
        print np.mean(final_clustors[0])
        print np.mean(final_clustors[1])
        print np.mean(final_clustors[2])
        print np.mean(final_clustors[3])
        print np.mean(final_clustors[4])
        
        print np.std(final_clustors[0])
        print np.std(final_clustors[1])
        print np.std(final_clustors[2])
        print np.std(final_clustors[3])
        print np.std(final_clustors[4])