...
class SocialNetwork(object):
    ...
    def createGraph(self):
        ...
        for row in c:
            g.add_edge(row[0], row[1], weight = row[2]/3000)
            #g.add_edge(row[0], row[1], weight = 1)

        estrong = [(u,v) for (u,v,d) in g.edges(data=True) if d["weight"] > 2]
        ...
        nx.draw(g)
        plt.axis("tight")
        fig.savefig ("C:/Users/n_shimada/Documents/networkxCi-Labo_onlySample.pdf")
        plt.show()
