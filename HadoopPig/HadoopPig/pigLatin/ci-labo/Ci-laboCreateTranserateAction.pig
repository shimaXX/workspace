...
%declare START_DATE  '2012-12-08' --つまり22日からのデータを取得する
%declare LAST_DATE  '2012-12-19' --つまり28日までのデータを取得する
...
%default PATH_INPUT_OCT 'works/input/aws/2012-12-15.gz';
%default PATH_OUTPUT 'works/output/Ci-Labo/Ci-LaboTranse1215Search';
...
--RowTypeData = LOAD '$PATH_INPUT_TYPE' USING PigStorage(',') 
--  AS (uid:chararray,type:chararray);

--(2)input dataの変数の選定
EditData = FOREACH RowData GENERATE ISOFormat(time) AS time , uid,
			GetMetaInfo(verb, ap) AS verb, cid, GetClient(ap) AS client;

--(3)フィルター処理
FilData = FILTER EditData BY (cid == 8 OR cid == 4 OR cid == 6 OR cid == 7)
 AND SUBSTRING(time, 0, 10) >= '$START_DATE' AND SUBSTRING(time, 0, 10) <= '$LAST_DATE';
