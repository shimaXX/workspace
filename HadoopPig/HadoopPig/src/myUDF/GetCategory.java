...
public class GetCategory extends EvalFunc<String> {
	...
	public String exec(Tuple input) throws IOException {
		...
		try {  
			...
			String cmpCategoryStr = "category_id";
			String sp_idKey = null;
			String output;
			
			for(int i=0; i<jo.size(); i++ ){
				String key = (String)keys.next();
				if(key.equals(cmpCategoryStr)){
					sp_idKey = (String)key;
					break;
				}else{
					sp_idKey = null;
	...
	public Schema outputSchema(Schema input) {
		try {
			...
			// バッグ内の項目の定義（項目名とデータ型）
			thisSchema.add(new Schema.FieldSchema("category", DataType.CHARARRAY));
